const configure = {
  ...
  getters: {
    ...
    currentListener: stat => {
      let currentListener = stat.currentListener
      if (!currentListener) {
        currentListener = JSON.parse(window.sessionStorage.getItem("user"))
      }
    }
  ...
  mutations: {
    ...
    setcurrentListener: (state, isActive) => {
      state.currentListener = currentListener
      window.sessionStorage.setItem('currentListener', JSON.stringify(currentListener))
    }
  }
}
